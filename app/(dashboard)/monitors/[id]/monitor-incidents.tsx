...
export function MonitorIncidents({ monitorId }: MonitorIncidentsProps) {
  ...
  useEffect(() => {
    async function fetchIncidents() {
      ...
      try {
        const response = await fetch(`/api/incidents?type=monitor&limit=10`, {
          headers: {
        ...
        // Filter incidents for this specific monitor
        const monitorIncidents = data.incidents.filter(inc => inc.source_id === monitorId);
    ...
    fetchIncidents();
  }, [monitorId, session?.user?.apiToken]);
