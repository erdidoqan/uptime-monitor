...
import { Check, Zap, Loader2, Clock, AlertCircle, CheckCircle, Info, ChevronDown, ChevronUp } from 'lucide-react';
...
export function HeroMonitorForm() {
  ...
  const [existingMonitor, setExistingMonitor] = useState<ExistingMonitor | null>(null);
  const [checkingExisting, setCheckingExisting] = useState(true);
  const [curlParsed, setCurlParsed] = useState(false);

  // Check if user already has a guest monitor
  useEffect(() => {
    async function checkExistingMonitor() {
      try {
        const response = await fetch('/api/guest-monitor');
        const data = await response.json();
        if (data.hasExistingMonitor) {
          setExistingMonitor(data.monitor);
        }
      } catch (err) {
        console.error('Failed to check existing monitor:', err);
      } finally {
  ...
  const handleUrlPaste = (e: React.ClipboardEvent<HTMLInputElement>) => {
    ...
    if (pastedText.trim().toLowerCase().startsWith('curl')) {
      ...
      if (parsed && parsed.url) {
        ...
        // Show advanced if headers or body was parsed
  ...
  async function handleTest() {
    ...
    try {
      ...
      const response = await fetch('/api/guest-monitor/test', {
        method: 'POST',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!response.ok) {
        setError(data.error || 'Monitör oluşturulamadı');
        return;
      }
      ...
      setError('Monitör oluşturulamadı. Lütfen tekrar deneyin.');
    } finally {
      setSubmitting(false);
    }
  }

  // Show existing monitor message
  if (!checkingExisting && existingMonitor) {
    return (
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="font-semibold text-white">
              {success ? 'Monitör Oluşturuldu!' : 'Aktif monitörünüz var'}
            </h3>
            <p className="text-sm text-gray-400">
              Her {existingMonitor.interval_sec / 60} dakikada kontrol ediliyor
            </p>
          </div>
        </div>

        <div className="p-3 rounded-lg bg-white/5 border border-white/10 mb-4">
          <p className="text-sm text-gray-300 font-mono truncate">{existingMonitor.url}</p>
          <p className="text-xs text-gray-500 mt-1">
            Bitiş: {new Date(existingMonitor.expires_at).toLocaleDateString('tr-TR')}
          </p>
        </div>

        <div className="space-y-3">
          <p className="text-sm text-gray-400">
            Daha fazla monitör oluşturmak veya bu monitörü kalıcı yapmak ister misiniz?
          </p>
  ...
  return (
    <div className="bg-[#1a1a1d]/90 backdrop-blur-xl rounded-2xl border border-white/10 p-6 shadow-2xl">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-400">
          Saniyeler içinde monitör oluşturun. Kayıt gerekmez.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        {showAdvanced && (
          <div className="space-y-4 pt-2 border-t border-white/10">
            {/* Method */}
            <div className="space-y-2">
              <Label className="text-sm font-medium text-gray-300">HTTP Metodu</Label>
              <Select
                value={form.method}
                onValueChange={(value) => setForm(prev => ({ ...prev, method: value }))}
              >
                <SelectTrigger className="bg-white/5 border-white/10 text-white focus:ring-0 focus:ring-offset-0">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
